...
for p in {0..12}; do
    if [ -d "storage/fnn-$p-50-100-20" ]; then
        printf "Model fnn-$p-50-100-20\n"
        python3.6 tagger.py --reset -fq
        cp storage/fnn-$p-50-100-20/* saved/
        python3.6 tagger.py --evaluate data/evaluation_known.txt -p $p -e 50 -s 100 -n 20 -i
        python3.6 tagger.py --evaluate data/evaluation_unknown.txt -p $p -e 50 -s 100 -n 20 -i
    fi
done
