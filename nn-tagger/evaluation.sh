...
for t in {0..12}; do
    if [ -d "storage/rnn-$t-50-1" ]; then
        printf "Model rnn-$t-50-1\n"
        python3.6 tagger.py --reset -fq
        cp storage/rnn-$t-50-1/* saved/
        python3.6 tagger.py --evaluate data/evaluation_known.txt -t $t -s 50 -n 1 -i
        python3.6 tagger.py --evaluate data/evaluation_unknown.txt -t $t -s 50 -n 1 -i
    fi
done

printf "\n## Hidden Layer Size:\n"
for s in 10 25 50 100 150 200 250 300 350 400 450 500; do
    if [ -d "storage/rnn-1-$s-1" ]; then
        printf "Model rnn-1-$s-1\n"
        python3.6 tagger.py --reset -fq
        cp storage/rnn-1-$s-1/* saved/
        python3.6 tagger.py --evaluate data/evaluation_known.txt -t 1 -s $s -n 1 -i
        python3.6 tagger.py --evaluate data/evaluation_unknown.txt -t 1 -s $s -n 1 -i
    fi
done
...
for n in 1 5 10 20 40 60 80 100; do
    if [ -d "storage/rnn-1-50-$n" ]; then
        printf "Model rnn-1-50-$n\n"
        python3.6 tagger.py --reset -fq
        cp storage/rnn-1-50-$n/* saved/
        python3.6 tagger.py --evaluate data/evaluation_known.txt -t 1 -s 50 -n $n -i
        python3.6 tagger.py --evaluate data/evaluation_unknown.txt -t 1 -s 50 -n $n -i
    fi
done