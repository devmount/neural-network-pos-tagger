...
# # FNN training | labels: fnn-p-e-s-n(-a)
...
# RNN training | labels: fnn-t-s-n(-a)
...
for t in {0..12}; do
    if [ ! -d "storage/rnn-$t-50-50-1" ]; then
        ...
        { python tagger.py --evaluate data/evaluation_known.txt -t $t -e 50 -s 50 -n 1 ; } > storage/rnn-$t-50-50-1/result_known.txt 2>&1
        { python tagger.py --evaluate data/evaluation_unknown.txt -t $t -e 50 -s 50 -n 1 ; } > storage/rnn-$t-50-50-1/result_unknown.txt 2>&1
        cp saved/* storage/rnn-$t-50-50-1/
...
for e in 1 5 10 25 50 100; do
    if [ ! -d "storage/rnn-1-$e-50-1" ]; then
        mkdir storage/rnn-1-$e-50-1
        { time python tagger.py --train data/training.corpus -t 1 -e $e -s 50 -n 1 ; } > storage/rnn-1-$e-50-1/training.txt 2>&1
        { python tagger.py --evaluate data/evaluation_known.txt -t 1 -e $e -s 50 -n 1 ; } > storage/rnn-1-$e-50-1/result_known.txt 2>&1
        { python tagger.py --evaluate data/evaluation_unknown.txt -t 1 -e $e -s 50 -n 1 ; } > storage/rnn-1-$e-50-1/result_unknown.txt 2>&1
        cp saved/* storage/rnn-1-$e-50-1/
        printf "Model with embedding size of %i is saved and evaluated.\n" $e
        python tagger.py --reset -fq
    fi
done
...
for s in 10 25 50 100 150 200; do
    if [ ! -d "storage/rnn-1-50-$s-1" ]; then
        mkdir storage/rnn-1-50-$s-1
        { time python tagger.py --train data/training.corpus -t 1 -e 50 -s $s -n 1 ; } > storage/rnn-1-50-$s-1/training.txt 2>&1
        { python tagger.py --evaluate data/evaluation_known.txt -t 1 -e 50 -s $s -n 1 ; } > storage/rnn-1-50-$s-1/result_known.txt 2>&1
        { python tagger.py --evaluate data/evaluation_unknown.txt -t 1 -e 50 -s $s -n 1 ; } > storage/rnn-1-50-$s-1/result_unknown.txt 2>&1
        cp saved/* storage/rnn-1-50-$s-1/
        printf "Model with hidden layer size of %i is saved and evaluated.\n" $s
        python tagger.py --reset -fq
    fi
done
...
for n in 1 5 10 20; do
    if [ ! -d "storage/rnn-1-50-50-$n" ]; then
        mkdir storage/rnn-1-50-50-$n
        { time python tagger.py --train data/training.corpus -t 1 -e 50 -s 50 -n $n ; } > storage/rnn-1-50-50-$n/training.txt 2>&1
        { python tagger.py --evaluate data/evaluation_known.txt -t 1 -e 50 -s 50 -n $n ; } > storage/rnn-1-50-50-$n/result_known.txt 2>&1
        { python tagger.py --evaluate data/evaluation_unknown.txt -t 1 -e 50 -s 50 -n $n ; } > storage/rnn-1-50-50-$n/result_unknown.txt 2>&1
        cp saved/* storage/rnn-1-50-50-$n/
        printf "Model trained with %i epochs is saved and evaluated.\n" $n
        python tagger.py --reset -fq
    fi
done
