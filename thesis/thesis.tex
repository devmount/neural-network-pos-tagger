...
To replace the existing tagger, a new tagger has to provide a class with two methods: \tt{train} and \tt{tag}. The \tt{train} method creates a new instance of the tagger class, trains it with the given training data and returns it. The training data itself must be a list of sentences, where a sentence is a list of tuples, containing each word of this sentence and its corresponding tag. Listing \ref{l.trainingdata} exemplifies the structure of the input data containing two tagged sentences.

\lstinputlisting[language=JSON, label={l.trainingdata}]{listings/method-tag.example}
\begingroup\captionof{lstlisting}{The structure of the input data containing two tagged sentences}\endgroup
