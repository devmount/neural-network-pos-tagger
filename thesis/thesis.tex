...
According to the evaluation results, the new tagger, which is proposed in this thesis, should provide the possibility to replace the initial HMM based part-of-speech tagger. To guarantee a seamless integration, the new tagger is implemented as a separate module with the same program interface that the old tagger already utilizes. This way, no other components of the conversational agent have to be changed and the effort of the replacement is kept to a minimum.
...
Figure \ref{f.rnn.structure} illustrates the architecture of an RNN, which was implemented for this thesis. Like the FNN, the RNN uses word embeddings as input features. It contains one hidden layer, which is populated with the embedding of the current input word on the one hand and by the output of the hidden layer of a previous training step on the other hand. This makes the RNN capable of memorizing information from the past. This capability can also be described as a long short-term memory (\i{LSTM}), emphasized in orange in Figure \ref{f.rnn.structure}. The output layer represents the set of existing POS tags and is connected to the hidden layer with weight matrix $W$.

The word embedding of each word of every sentence in the training corpus represents the input feature vector, which is propagated to the hidden and the output layer together with information from previous training steps. Similar to the processing of FNNs, the POS tag is predicted for the current word and the current loss and accuracy are calculated. After propagating the error back to adjust the weights and reduce the loss, the next training step is executed, taking the current training step into account.
...
\begin{figure}[ht]
	...
	\caption[Structure of a Recurrent Neural Network]{The structure of a recurrent neural network. The feature vector is the initial vector of the corresponding input word at time $t$. The output of the hidden layer from previously trained words (here at time $t-x$) is populated back into the same hidden layer for the current word. $V$, $X$ and $W$ are weight matrices with respective matrix dimensions of $\textsf{embedding size} \times \textsf{hidden layer size}$ for $V$, $\textsf{hidden layer size} \times \textsf{hidden layer size}$ for $X$ and $\textsf{hidden layer size} \times \textsf{number of POS tags}$ for $W$.}
	\vspace{3mm}
...
The implementation of the proposed RNN architecture utilizes the TensorFlow library as well. Besides the embedding of the current word, the feature vector now has an additional dimension describing the number of past training steps that are considered for the current training step.
...
For the training of the RNN models, the following 5 parameters were considered:
...
\begin{itemize}
	...
	\item \b{Embedding size} (\i{\b{e}}) describes the dimension of the word embeddings (as in the FNN)
...
Based on these 5 parameters, Table \ref{t.training.tuning.rnn} shows different parameter combinations. The emphasized cells show the respective parameter that is variable. The resulting number of models is 47; however, due to an overlap in configurations of the training groups, the total number of models to be trained with the RNN is 44.

All in all, a total of 140 distinct models are trained, given the parameter specification for the two neural network architectures above.

\begin{table}[H]
	\centering\small\def\arraystretch{1.5}\begin{tabular}{ c c >{\centering}p{20mm} >{\centering}p{20mm} c c c }
	\trule
	\textsc{Group} & $t$ & $e$ & $s$ & $n$ & $a$ & \textsc{Models} \\
	\drule
	\i{9} & \b{1--10} & 50 & 50 & 1 & relu & \i{10} \\
	\mrule
	\i{10} & 1 & \b{1, 5, 10, 20, 50--400}\tablefootnote{With a step size of 50\label{fiftyrnn}} & 50 & 1 & relu & \i{12} \\
	\mrule
	\i{11} & 1 & 50 & \b{10, 25, 50--500}\footref{fiftyrnn} & 1 & relu & \i{12} \\
	\mrule
	\i{12} & 1 & 50 & 50 & \b{1, 5--20}\tablefootnote{With a step size of 5\label{fivernn}} & relu & \i{5} \\
	\srule
	\i{13} & 6 & 50 & 50 & 1 & \b{relu, ...}\tablefootnote{For the RNN the same 8 activation functions as for the FNN are used: \ftt{RELU}, \ftt{RELU6}, \ftt{ELU}, \ftt{SIGMOID}, \ftt{TANH}, \ftt{SELU}, \ftt{SOFTPLUS} and \ftt{SOFTSIGN}. See Chapter \ref{c.postagging.fnn.architecture}} & \i{8} \\
	\brule
	\end{tabular}
	\vspace{3mm}
	\caption[Parameter combinations of RNN Models]{The parameter configuration for the training of RNN models. The rows represent the training groups with the corresponding variable parameter (in bold), while the columns represent the single training parameters (\i{\b{t}} (number of time steps), \i{\b{e}} (embedding size), \i{\b{s}} (hidden layer size), \i{\b{n}} (number of training epochs), \i{\b{a}} - activation function). The first column denotes the group number (for referencing training groups) whereas the last column shows the resulting number of models for each training group.}
	\label{t.training.tuning.rnn}
\end{table}

% ===================================================================================
\chapter{Evaluation and Comparison}\label{c.evaluation}
After giving a short description of how the evaluation tests were created, this chapter provides the evaluation results of all models of the 13 training groups that were trained using the variation of training parameters previously described. In addition to the results of each training group, the three different architectures (FNN, RNN and HMM) are compared.
...
\b{Training group 2} included the evaluation based on 18 different values for the embedding size $e$ of the input words for the FNN. The results are presented in Figure \ref{f.evaluation.fnn.e}.

\begin{figure}[H]
	\hspace{-5mm}\includegraphics[width=1.07\textwidth]{images/evaluation_fnn_e}
	\caption[FNN Evaluation: Embedding Size]{The evaluation results of the FNN for parameter $e$: the embedding size.}
	\label{f.evaluation.fnn.e}
\end{figure}

With a few exceptions, the accuracy for the known test  increases slightly with larger embeddings, reaching a maximum of 90.5\% (and $\kappa=0.801$) on an embedding size of 600. The highest accuracy (70.1\%, $\kappa=0.639$) for the unknown test was reached by the model with an embedding size of 450.
...
The first training group for the RNN, \b{training group 9}, contains models trained with 10 different time steps $t$. The evaluation result is presented in Figure \ref{f.evaluation.rnn.t}.

\begin{figure}[H]
	\hspace{-5mm}\includegraphics[width=1.07\textwidth]{images/evaluation_rnn2_t}
...
Overall, all models based on the different time steps achieved similar results on both tests without a discernible tendency. However, for both tests, the models with $t=4$ and $t=6$ stands out with the highest accuracy of 81.3\% and $\kappa=0.728$ for the known test and 68.4\% and $\kappa=0.631$ for the unknown test for $t=6$.

\newpage

\b{Training group 10} included the evaluation based on 12 different values for the embedding size $e$ of the input words for the RNN. The results are presented in Figure \ref{f.evaluation.rnn.e}.

\begin{figure}[H]
	\hspace{-5mm}\includegraphics[width=1.07\textwidth]{images/evaluation_rnn2_e}
	\caption[RNN Evaluation: Embedding Size]{The evaluation results of the RNN for parameter $e$: the embedding size.}
	\label{f.evaluation.rnn.e}
\end{figure}

With a few exceptions, the accuracy for both tests increases slightly with larger embeddings, reaching a maximum of 81.6\% ($\kappa=0.723$) for the known test and 68.9\% ($\kappa=0.635$) for the unknown test utilizing an embedding size of 300. Models with embedding sizes of 150 and 400 achieved a similar but slightly lower accuracy.

\newpage

The evaluation results of \b{training group 11}, which varied the size of the hidden layer $s$ is illustrated in Figure \ref{f.evaluation.rnn.s}.

\begin{figure}[H]
	\hspace{-5mm}\includegraphics[width=1.07\textwidth]{images/evaluation_rnn2_s}
...
In general, models with a higher hidden layer size achieved a higher accuracy on both tests. Evaluating the known test, the highest accuracy of 81.8\% ($\kappa=0.726$) was reached by a model with hidden layer size $s=400$. For the unknown test, the model with $s=250$ tagged the most words correctly (69.1\%, $\kappa=0.632$). Nevertheless, the highest kappa value $\kappa=0.636$ was assigned to the model with $s=400$ again, which achieved a slightly lower accuracy of 68.98\%.

\newpage

\b{Training group 12} included the evaluation based on 5 different numbers of training epochs $e$. The results are presented in Figure \ref{f.evaluation.rnn.n}.

\begin{figure}[H]
	\hspace{-5mm}\includegraphics[width=1.07\textwidth]{images/evaluation_rnn2_n}
...
For both tests, the model with $n=5$ achieved the highest number of correctly tagged words: 82.2\% ($\kappa=0.721$) for the known test and 69.5\% ($\kappa=0.642$) for the unknown test. For higher numbers of training epochs ($s>5$), the accuracy decreases for more than 5\%.

\newpage

The last training group, \b{training group 13}, refers to parameter $a$: the activation function (see Chapter \ref{c.postagging.fnn.architecture}). The result for the accuracy of 8 models that were trained with a different activation function each is presented in Figure \ref{f.evaluation.rnn.a}. The configuration of the time step parameter was based on the results of training group 9, using $t=6$. The embedding size ($e=50$) size of the hidden layer ($s=50$) and the number of training epochs ($n=1$) were chosen with respect to a reasonable training time.

\begin{figure}[H]
	\hspace{-5mm}\includegraphics[width=1.07\textwidth]{images/evaluation_rnn2_a}
	\caption[RNN Evaluation: Activation Function]{The evaluation results of the RNN: Cohen's Kappa for parameter $a$: the type of activation function.}
	\label{f.evaluation.rnn.a}
\end{figure}

The best evaluation results of this training group regarding accuracy were achieved by the model utilizing the \tt{RELU} activation function. For the known test, an accuracy of 81.3\% ($\kappa=0.728$) and for the unknown test, an accuracy of 68.4\% ($\kappa=0.631$) was achieved. The other models of this training group reached an accuracy with a maximum of 77.1\%.

\newpage

Considering the evaluation results of all 5 training groups for the RNN, the model with the following parameter configuration was found to achieve the highest accuracy: one time step, embedding size of 50, hidden layer size of 50, 5 training epochs and \tt{RELU} as activation function.
...
\begin{figure}[H]
\centering
\subcaptionbox{Accuracy of correctly tagged words\label{f.evaluation.hmm.a}}
{\includegraphics[width=0.495\textwidth]{images/evaluation_hmm}}
\subcaptionbox{Cohen's Kappa\label{f.evaluation.hmm.k}}
...
{\includegraphics[width=0.495\textwidth]{images/comparison2}}
\subcaptionbox{Comparison of the kappa score\label{f.evaluation.comparison.k}}
{\includegraphics[width=0.495\textwidth]{images/comparison2_k}}
\vspace{1em}
...
The FNN model achieved the highest accuracy as well as the highest kappa score for the known test, closely followed by the HMM2 model, which was only slightly less accurate. The difference between these two models is only 0.2\% ($\Delta\kappa=0.017$). With a kappa score of more than 0.8, both models offer a very good agreement with the labeled test data. However, the RNN and the HMM1 models still reached a good agreement, tagging more than 80\% of the words correctly.

The result of the unknown test is a bit more distinctive. The highest accuracy as well as the highest kappa score was reached by the HMM2 model, followed by the FNN model with a difference of 7.28\% ($\Delta\kappa=0.084$). With a kappa score of more than 0.6, both models as well as the RNN model offer a good agreement with the labeled test data. Although the FNN model performed better than the RNN model, the differences between the results of both are very small (2.53\%, $\Delta\kappa=0.012$). The FNN1 model reaches a moderate agreement with the labeled test data, tagging less than two-thirds of the words correctly.

Overall, every model achieved a higher accuracy and a higher kappa score on the know test than on the unknown test. While this is an expected result, the differences between the accuracy of known and unknown test vary for each model. While this difference is smaller for the HMM2 and RNN model (13.14\% and 12.74\%), the HMM1 model shows a difference of 17.88\%. The largest difference can be observed in the FNN model, which tagged an additional 20.63\% of the words of the known test correctly, compared to those of the unknown test.
...
After evaluating the trained language models, it could be shown that the HMM based on new training data significantly performed better than the previous HMM tagger, especially on unknown data. It was also shown that a tagger based on a feed-forward neural network was able to outperform this new HMM tagger on known evaluation data. Both, feed-forward and recurrent neural network models, achieved a higher accuracy than the HMM \Alex\ utilized before.

\section{Discussion}\label{c.conclusion.discussion}
According to the evaluation results, the FNN model achieved the highest accuracy for known data and would therefore be preferred for productive use. However, the HMM model based on the new training corpus (HMM2) performed significantly better on unknown data, showing the ability to generalize better. Given the fact that a chatbot has to deal with a lot of unknown input data due to users that do not know the limits of the chatbots knowledge and capabilities, the result of the unknown test is more important for the choice of the language model to use in \Alex.

When reviewing the comparison of the two hidden Markov models for the unknown test, it is apparent that the quality of the training data is of great importance. The improvements on the training corpus proposed in this thesis caused a higher accuracy in word tagging of more than 16\%. However, the training data still has a fixed structure due to its origin in the sentence templates, which is a possible reason for the lower accuracy of the neural network approaches on the unknown data. The effect of the structure of the training data is especially evident in the results for the recurrent neural network, which created less accurate language models compared to the FNN.
...
In this thesis, parameters that were thought to influence the tagging results, such as the number of previous words, the number of time steps, the size of the hidden layer, the size of the word embeddings, the number of training epochs and the type of activation function were analyzed. Yet there are additional parameters, which could also be examined to improve the accuracy further. For the FNN, parameters of possible interest are the number of subsequent words (in addition to the number of previous words), the number of hidden layers and the training optimizer. The use of multiple LSTM cells for the RNN could be interesting too, as well as the training optimizer, which are possible additional parameters. Moreover, an exponential decay for the learning rate of the optimizer, especially for the RNN, could be explored.
...
\begin{figure}[H]
	...
	\caption[FNN Evaluation: Embedding Size]{The evaluation results of the FNN: Cohen's Kappa for parameter $e$: the embedding size.}
...
\begin{figure}[H]
	...
	\caption[FNN Evaluation: Embedding Size for a larger Hidden Layer]{The evaluation results of the FNN: Cohen's Kappa for parameter $e$: the embedding size with a higher hidden layer $s=350$.}
...
\begin{figure}[H]
	...
	\caption[FNN Evaluation: Hidden Layer for larger Embeddings]{The evaluation results of the FNN: Cohen's Kappa for parameter $s$: the hidden layer size for larger embeddings $e=250$.}
...
\begin{figure}[H]
	...
	\caption[FNN Evaluation: Activation Function]{The evaluation results of the FNN: Cohen's Kappa for parameter $a$: the activation function.}
...
\vspace{-13mm}
\begin{figure}[H]
	\centering\includegraphics[width=\textwidth]{images/evaluation_rnn2_t_k}
	\caption[RNN Evaluation: Number of Time Steps]{The evaluation results of the RNN: Cohen's Kappa for parameter $t$: the number of time steps.}
...
\vspace{-13mm}
\begin{figure}[H]
	\centering\includegraphics[width=\textwidth]{images/evaluation_rnn2_e_k}
	\caption[RNN Evaluation: Embedding Size]{The evaluation results of the RNN: Cohen's Kappa for parameter $e$: the size of the word embeddings.}
	\label{f.evaluation.rnn.e.k}
\end{figure}

\vspace{-13mm}
\begin{figure}[H]
	\centering\includegraphics[width=\textwidth]{images/evaluation_rnn2_s_k}
	\caption[RNN Evaluation: Hidden Layer Size]{The evaluation results of the RNN: Cohen's Kappa for parameter $s$: the size of the hidden layer.}
...
\vspace{-11mm}
\begin{figure}[H]
	\centering\includegraphics[width=\textwidth]{images/evaluation_rnn2_n_k}
	\caption[RNN Evaluation: Number of Training Epochs]{The evaluation results of the RNN: Cohen's Kappa for parameter $n$: the number of training epochs.}
...
\vspace{-11mm}
\begin{figure}[H]
	\centering\includegraphics[width=\textwidth]{images/evaluation_rnn2_a_k}
	\caption[RNN Evaluation: Activation Function]{The evaluation results of the RNN: Cohen's Kappa for parameter $a$: the type of activation function.}
