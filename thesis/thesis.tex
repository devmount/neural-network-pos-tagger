...
The hidden layer is built using \tt{tf.nn.rnn\_cell.LSTMCell()}\footnote{This function uses an implementation proposed by S. Hochreiter et. al. in 1997 \cite{hochreiter1997}.}, a function that creates a long short-term memory (LSTM) recurrent network cell which is then used in \tt{tf.nn.dynamic\_rnn()} to compute the logits for the output layer.
...
Furthermore, wording referring to linking words and actions, which occurs in the logs, was improved with the help of inline choices. The following table shows those improvements, comparing the previous and the new sentence template:
